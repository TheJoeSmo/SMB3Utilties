...
static int verify_generated_files() {
    ...
    // Get the timestamp for the ASM; this is to be sure we generated a new FNS
    // and NES file
    ...
    if (stat(_buffer, &stat_buf) == -1) {
        snprintf(
            _error_msg,
            ERROR_MSG_LEN,
            "Initial test build FAILED:\nUnable to stat %s (symbol listing "
            "file not created?)",
            _buffer);
        return 0;
    }
    ...
    if (stat_buf.st_mtime < asm_mtime) {
        snprintf(
            _error_msg,
            ERROR_MSG_LEN,
            "Initial test build FAILED:\n%s is older than %s" EXT_ASM " (symbol listing file not generated?)",
            _buffer,
            NoDice_config.filebase);
        return 0;
    }
    ...
    if (stat_buf.st_mtime < asm_mtime) {
        snprintf(
            _error_msg,
            ERROR_MSG_LEN,
            "Initial test build FAILED:\n%s is older than %s" EXT_ASM " (ROM file not generated?)",
            _buffer,
            NoDice_config.filebase);
        return 0;
    }
