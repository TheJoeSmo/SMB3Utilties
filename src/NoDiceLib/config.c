...
static void parse_generators(ezxml_t root, struct NoDice_generator *generator, int base_index, int gen_type) {
    ...
    for (node = ezxml_child(root, "generator"); node != NULL; node = ezxml_next(node)) {
        ...
        for (i = 0, pnode = ezxml_child(node, "param"); i < GEN_MAX_PARAMS && pnode != NULL;
             i++, pnode = ezxml_next(pnode)) {
...
static void parse_header_options(ezxml_t root, struct NoDice_headers *header) {
    ...
    if ((header->options_list_count = count_children(root, "options")) > 0) {
        ...
        header->options_list =
            (struct NoDice_header_options *) calloc(header->options_list_count, sizeof(struct NoDice_header_options));
        ...
        for (node = ezxml_child(root, "options"); node != NULL; node = ezxml_next(node)) {
            ...
            if ((this_options->options_count = count_children(node, "option")) > 0) {
                ...
                this_options->options =
                    (struct NoDice_option *) calloc(this_options->options_count, sizeof(struct NoDice_option));
...
static void parse_objects(ezxml_t root, struct NoDice_objects *objects) {
    ...
    if (objects_count > 0) {
        ...
        for (node = ezxml_child(root, "object"); node != NULL; node = ezxml_next(node)) {
            ...
            this_obj->sprites = (struct NoDice_object_sprites *) calloc(
                1,
                sizeof(struct NoDice_object_sprites) * this_obj->total_sprites);
...
int _config_init() {
...

    {
        ...
        strcpy(NoDice_config.buildinfo._build_str,
               build);                                    // Start with build string
        strcat(NoDice_config.buildinfo._build_str, " ");  // Append a space
        strcat(NoDice_config.buildinfo._build_str,
               _buffer);  // Append the [game].asm
    ...
    if (((game_xml = ezxml_parse_file(_buffer)) == NULL) || (game_xml->name == NULL)) {
        ...
        for (node = ezxml_child(game_node, "configitem"); node != NULL; node = ezxml_next(node)) {
            ...
            const char *value = ezxml_attr(node, "value");
        ...
        parse_tilehints(
            ezxml_child(game_node, "tilehints"),
            &NoDice_config.game.tilehints,
            &NoDice_config.game.tilehint_count);
        ...
        if (NoDice_config.game.tileset_count > 0) {
            ...
            NoDice_config.game.tilesets =
                (struct NoDice_tileset *) calloc(1, sizeof(struct NoDice_tileset) * NoDice_config.game.tileset_count);
            ...
            for (node = ezxml_child(game_node, "tileset"); node != NULL; node = ezxml_next(node)) {
                ...
                tileset->gen_count = (var_count = count_children(ezxml_child(node, "vargenerators"), "generator")) +
        ...
        if (NoDice_config.game.total_map_object_items > 0) {
            ...
            NoDice_config.game.map_object_items = (struct NoDice_map_object_item *) calloc(
                1,
                sizeof(struct NoDice_map_object_item) * NoDice_config.game.total_map_object_items);
        ...
        if ((game_node = ezxml_child(game_xml, "maptiles")) != NULL) {
            ...
            if (NoDice_config.game.total_map_special_tiles > MAX_SPECIAL_TILES) {
                snprintf(
                    _error_msg,
                    sizeof(_error_msg),
                    "Maximum definable <maptiles> <tile> blocks is %i "
                    "(this is an internal editor limit)",
                    MAX_SPECIAL_TILES);
                return 0;
            }
            ...
            if (NoDice_config.game.total_map_special_tiles > 0) {
                ...
                // Allocate that many map items
                NoDice_config.game.map_special_tiles = (struct NoDice_map_special_tile *) calloc(
                    1,
                    sizeof(struct NoDice_map_special_tile) * NoDice_config.game.total_map_special_tiles);
...
const char *NoDice_config_game_add_level_entry(
    ...
    for (node = ezxml_child(game_node, "tileset"); node != NULL; node = ezxml_next(node)) {
        ...
        if (tileset == id) {
            ...
            // We're at the <levels /> ...
            // Since NoDice uses the XML's string memory and relies on
            ...
            if (node = ezxml_add_child_d(game_node, "level", 0)) {
                ...
                // Pointers may have changed, memory moved, etc... gotta
                ...
                // (which creates the files, etc.)

                // Clear the level structure so no spurious things get
                // saved
