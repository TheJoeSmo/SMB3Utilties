...
// Random macros that were in M6502.c
#define M_PUSH(Rg)             \
...
#define M_ADC(Rg)                                                                                            \
    if (R->P & D_FLAG) {                                                                                     \
        ...
        R->P |= (~(R->A ^ Rg) & (R->A ^ K.B.l) & 0x80 ? V_FLAG : 0) | (K.B.h ? C_FLAG : 0) | ZNTable[K.B.l]; \
        R->A = K.B.l;                                                                                        \
    }
...
#define M_SBC(Rg)                                                                                           \
    if (R->P & D_FLAG) {                                                                                    \
        ...
        K.W = R->A - Rg - (~R->P & C_FLAG);                                                                 \
        R->P &= ~(N_FLAG | V_FLAG | Z_FLAG | C_FLAG);                                                       \
        R->P |= ((R->A ^ Rg) & (R->A ^ K.B.l) & 0x80 ? V_FLAG : 0) | (K.B.h ? 0 : C_FLAG) | ZNTable[K.B.l]; \
        R->A = K.B.l;                                                                                       \
    }
...
// Scratch space for modified levels; situated at 0x4020-0x5FFF ("expansion
// ROM") Only available when is_loading_level is set, not part of stock SMB3
...
#define Vert_Scroll_Hi \
    Rd6502(_ram[VERT_SCROLL_HI].address)                             // Provides a "High" byte for vertically scrolling
                                                                     // (only used during vertical levels!)
...
#define TileAddr_Off \
    Rd6502(_ram[TILEADDR_OFF].address)                     // During level loading, specifies an offset into the
                                                           // current Mem_Tile_Addr setting
...
#define World_Num \
    Rd6502(_ram[WORLD_NUM].address)  // Current world index (0-8, where 0 = World
                                     // 1, 7 = World 8, 8 = World 9 / Warp Zone)
#define PalSel_Tile_Colors \
    Rd6502(_ram[PALSEL_TILE_COLORS].address)  // Stores value to index which tile color set to use
                                              // when palette loading routine is called
#define PalSel_Obj_Colors \
    Rd6502(_ram[PALSEL_OBJ_COLORS].address)  // Stores value to index which object color set to use
                                             // when palette loading routine is called
#define Map_Objects_Itm(index) Rd6502(_ram[MAP_OBJECTS_ITM].address + (index))
#define Level_AltTileset \
    Rd6502(_ram[LEVEL_ALTTILESET].address)  // Pointer to level's "alternate" tileset (when you go
                                            // into bonus pipe, etc.)
#define Level_BG_Page1_2 \
    Rd6502(_ram[LEVEL_BG_PAGE1_2].address)  // Sets which bank the first and second page (2K / 64
                                            // 8x8 tiles) of BG is using
...
static unsigned short vram_virtual_x(unsigned short addr) {
    ...
    // Divide offset by size of a single horizontal screen; each screen is
    // SCREEN_WIDTH tiles Multiply by SCREEN_WIDTH to get the absolute virtual
    // column
...
static unsigned short vram_virtual_x_v(unsigned short addr) {
    ...
    // Column can always be determined by the lower nibble of the lower byte of
    // the offset
    x = (offset & 0xF);
...
byte Rd6502(register word Addr) {
    if (is_loading_level) {
        if ((Addr == LeveLoad_Generators) || (Addr == LeveLoad_FixedSizeGens) || (Addr == LoadLevel_StoreJctStart)) {
            ...
            struct NoDice_the_level_generator *g =
            ...
            if (Addr == LeveLoad_Generators) {
                ...
                // p2 MAY be used, next byte after the base generator was
            ...
            if (cur_gen == NULL) {
                ...
                // For all subsequent generators, the index is the previous
                // index + 1
                g->index = g->prev->index + 1;
            }

            // Mark start of generator
            // NOTE: -3 because by the execution flow, by the time it has
            // decided which routine to call, it has already read in the 3
            // primary bytes required by all generators, so the actual start is
            // 3 bytes ago...
...
static void rom_Reset6502(int ram_clear) {
    // Set up default ROM banks -- _PRG_A and _PRG_D are always (for SMB3) the
    // last two PRG banks The middle are 00 and 01 until otherwise
...
int _rom_load() {
    ...
    // Get number of 8KB CHR pages and multiply to find number of 1KB CHR banks
    // (MMC3 standard)
...
int NoDice_PRG_refresh() {
    ...
    if (PRG_size != PRG_size_check) {
        snprintf(
            _error_msg,
            ERROR_MSG_LEN,
            "Failed to refresh PRG cache because PRG changed size?!  Recommend "
            "shutting down and restarting NoDice if modifying ROM header!");
        return 0;
    }
...
unsigned short NoDice_get_addr_for_label(const char *label) {
    // FIXME: This should be redone as a sorted array so we can implement a
    // binary search!!
...
void NoDice_load_level(unsigned char tileset, const char *level_layout, const char *object_layout) {
    ...
    if (tileset > 0) {
        ...
        address = (unsigned short) strtoul(level_layout, NULL, 0) -
                  1;  // This actually specifies the world number (zero-based)
        object_address = 0;
    }
...
void NoDice_load_level_by_addr(unsigned char tileset, unsigned short address, unsigned short object_address) {
    ...
    if (tileset > 0) {
        ...
        // The last generator doesn't get a chance to be patched, so we'll do it
        // now
        prev_gen_patch(0);
    } else {
    ...
    if (tileset > 0) {
        ...
        NoDice_the_level.header.vert_scroll = (!NoDice_the_level.header.is_vert)
                                                  ? Vert_Scroll
                                                  : (Vert_Scroll_Hi * SCREEN_BYTESIZE_V / SCREEN_WIDTH * TILESIZE);
    ...
    if (tileset > 0) {
        ...
        /*
        {
            ...
            {
                fprintf(f, "%s\tid = %02i\taddr = %04X/%04X/%04X %i, %i to %i,
        %i\tsize = %i\tp1 = $%02X\tp2 = $%02X\n", names[cur->type], cur->id,
        cur->addr_start, cur->addr_min, cur->addr_max, cur->xs, cur->ys,
        cur->xe, cur->ye, cur->size, cur->p[0], cur->p[1]);
            }
        ...
        */
        ...
        if ((address + 1) != strtoul(NoDice_config.game.options.warpzone, NULL, 0)) {
            for (i = 0; i < MOBJS_MAX; i++) {
                ...
                // Technically map objects have full pixel placement
                // possibility, but this is never used, and for now this is more
                // compatible with the existing editor structure.
                object->id = Map_Objects_ID(i);
                object->col = x / TILESIZE;
                object->row = (Map_Objects_Y(i) / TILESIZE) - MAP_OBJECT_BASE_ROW;  // Slight adjustment since there's
...
static void vaddr_to_level(unsigned short addr, unsigned char *t15, unsigned char *t16) {
    ...
    *t15 |= ((inter_screen_offset >> 4) & 0x0F);
    ...
    // Lower 4 bits of Temp_Var16 are the lower 4 bits of the inter-screen
    // offset
    ...
    while (gen != NULL) {
        ...
        switch (gen->type) {
            case GENTYPE_JCTSTART:
                ...
                // Upper 3 bits of Temp_Var15 are ALL set... i.e. Temp_Var15 =
                // 111x xxxx The lower 4 bits specify an index into the junction
                // starts
                t15 = 0xE0 | (gen->id & 0xF);
                *ptr++ = t15;
            ...
            case GENTYPE_VARIABLE:
                ...
                // Upper 3 bits of Temp_Var15 select a multiple of 15 for the
                // generator
                ...
                *ptr++ = t16;
                ...
                // The upper 4 bits of LL_ShapeDef must NOT be zero, or else
                // that specifies a fixed-size generator; the loading code in
                // SMB3 compensates for that by requiring LL_ShapeDef > 0, hence
                // why a subtraction of 1 is required and also why it's a
                // multiple of 15, not 16! The lower 4 bits of LL_ShapeDef are
                // the first parameter (range 0 to 15)
                *ptr++ = (((gen->id % 15) + 1) & 0x0F) << 4  // Upper 4 bits
                         | (gen->p[0] & 0x0F);               // Lower 4 bits

                // If this generator is occupying more bytes than we can
                // tolerate, abort the execution now...
                if (gen->size >= 3 + GEN_MAX_PARAMS) {
                    NoDice_Run6502_Stop = RUN6502_GENERATOR_TOO_LARGE;
                    break;
                }

                // Some generators may have additional parameters; commit them
...
int NoDice_get_music_context(
    ...
    // Headers:
    // Music_xxx_IndexOffs	-- defines index into Music_xxx_Headers for given
    // Start/End/Loop index Music_xxx_Headers

    // Music spans page 28 and 29
    rom_MMC3_set_pages(28,
                       29);  // FIXME: Hardcoded, should probably allow others
    ...
    for (segment = context->index_start; segment <= context->index_end; segment++) {
        ...
        // 0: Music_RestH_Base value (always divisible by $10; base part of
        // index into PRG031's Music_RestH_LUT) 1-2: Address of music segment
        // data (all tracks this segment, offsets to follow, except implied
        // Square 2 zero) 3: Triangle track starting offset ($00 means disabled)
        ...
        this_segment->segment_data = rom_make_ptr_for_addr(
            MAKE16(Rd6502(Music_Headers + header_offset + 2), Rd6502(Music_Headers + header_offset + 1)));
