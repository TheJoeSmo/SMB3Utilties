...
static void showif_handler_count_foreach_callback(gpointer data, gpointer user_data) {
    ...
    struct showif_handler_foreach_callback_data *showif_foreach_data =
...
void gui_generate_option_controls(
    GList **context,
    GtkWidget *vbox,
    unsigned short header_byte,
    unsigned char header_val,
    const struct NoDice_headers *header,
    GCallback list_change,
    GCallback toggle_change,
    GCallback spin_change) {
    int j;
    ...
    for (j = 0; j < header->options_list_count; j++) {
        ...
        struct gui_generate_option_controls_context *context_obj =
            (struct gui_generate_option_controls_context *) malloc(sizeof(struct gui_generate_option_controls_context));
