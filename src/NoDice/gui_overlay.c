...
static void gtk_selectable_overlay_realize(GtkWidget *widget) {
    ...
    attributes.event_mask = gtk_widget_get_events(widget) | GDK_EXPOSURE_MASK | GDK_ENTER_NOTIFY_MASK |
    ...
    if (event->type == GDK_BUTTON_PRESS) {
        if (event->button == 1) {
            ...
            gtk_container_foreach(
                GTK_CONTAINER(widget->parent),
                gtk_selectable_overlay_release_foreach_callback,
                (gpointer) widget);
...
static gboolean gtk_selectable_overlay_release(GtkWidget *widget, GdkEventButton *event) {
    ...
    if (event->type == GDK_BUTTON_RELEASE && event->button == 1) {
        ...
        diff_row =
            (int) ((double) (alloc.y - selectable_overlay->translation_data.origin_y) / gui_draw_info.zoom / TILESIZE);
        diff_col =
...
static gboolean gtk_selectable_overlay_motion(GtkWidget *widget, GdkEventMotion *event) {
    ...
    GtkSelectableOverlay *selectable_overlay = GTK_SELECTABLE_OVERLAY(widget);
    int diff_x =
        (int) ((event->x - selectable_overlay->translation_data.mouse_origin_x) / gui_draw_info.zoom / TILESIZE);
    int diff_y =
        (int) ((event->y - selectable_overlay->translation_data.mouse_origin_y) / gui_draw_info.zoom / TILESIZE);

    if (diff_x != 0 || diff_y != 0) {
        gtk_widget_get_allocation(widget, &alloc);

        gtk_fixed_move(
            GTK_FIXED(widget->parent),
            widget,
            alloc.x + diff_x * TILESIZE * gui_draw_info.zoom,
            alloc.y + diff_y * TILESIZE * gui_draw_info.zoom);
    }
