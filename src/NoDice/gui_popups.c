...
int gui_open_level_popup(
    ...
    GtkWidget *popup = gtk_dialog_new_with_buttons(
        (popup_options & OLP_FORBROWSE) ? "Browse for Level" : "Open Level from ROM",
        GTK_WINDOW(gui_main_window),
...

    {
        ...
        // Left - List of levels
        {
            ...
            gtk_text_view_set_wrap_mode(
                GTK_TEXT_VIEW(gui_open_level_popup_desc_widgets.textbox_desc),
                GTK_WRAP_WORD_CHAR);
            gtk_text_view_set_editable(GTK_TEXT_VIEW(gui_open_level_popup_desc_widgets.textbox_desc), FALSE);

            level_list =
                gui_listbox_new(G_CALLBACK(gui_open_level_popup_change_level), &gui_open_level_popup_desc_widgets);
...
static void gui_level_properties_browse_click(GtkButton *button, gpointer user_data) {
    ...
    NoDice_the_level.level = edit_level_find(
    ...
    if (gui_open_level_popup(OLP_FORBROWSE | OLP_NIBBLE_TILESETS_ONLY, &tileset, &level)) {
        ...
        snprintf(
            buffer,
            sizeof(buffer),
            "You MUST now change the 'Tileset of Alternate Level' in the property window to 'Tileset %i (%s)' or the "
            "game will crash!",
            the_tileset->id,
            the_tileset->name);
...
void gui_level_properties(struct NoDice_the_level_generator *selected_gen) {
...

    {
        ...
        const struct NoDice_the_levels *alt_level = edit_level_find(
            NoDice_the_level.header.alt_level_tileset,
            NoDice_the_level.header.alt_level_layout,
            NoDice_the_level.header.alt_level_objects);
        ...
        g_signal_connect(
            ...
            "clicked",
            G_CALLBACK(gui_level_properties_browse_click),
            (gpointer) alt_level_label);
        ...
        for (i = 0; i < LEVEL_HEADER_COUNT; i++) {
            ...
            gui_generate_option_controls(
                ...
                vbox,
                i,
                NoDice_the_level.header.option[i],
                header,
                G_CALLBACK(gui_level_properties_option_list_change),
                G_CALLBACK(gui_level_properties_option_toggle_change),
                G_CALLBACK(gui_level_properties_option_spin_change));
        }
...
static void gui_special_obj_properties_id_change(GtkComboBox *widget, gpointer user_data) {
    ...
    gui_generate_option_controls(
        ...
        data->vbox,
        (unsigned short) data->object_index,
        object->row,
        &this_obj->special_options,
        G_CALLBACK(gui_special_obj_properties_option_list_change),
        G_CALLBACK(gui_special_obj_properties_option_toggle_change),
        G_CALLBACK(gui_special_obj_properties_option_spin_change));
...
int gui_special_obj_properties(struct NoDice_the_level_object *object) {
...

    {
        ...
        g_signal_connect(
            special_objs_list,
            "changed",
            G_CALLBACK(gui_special_obj_properties_id_change),
            (gpointer) &data);
...
int gui_map_obj_properties(struct NoDice_the_level_object *object) {
...

    {
        ...
        g_signal_connect(
            map_items_list,
            "changed",
            G_CALLBACK(gui_map_obj_properties_item_change),
            &NoDice_the_level.map_object_items[obj_index]);
...
#define MAPLINKIDX_PACK(ovrtype, special_tile_def_index, map_link_index) \
    (((ovrtype) << 14) | (((special_tile_def_index) &0x003F) << 8) | ((map_link_index) &0x00FF))
...
static const struct NoDice_header_options *opt_list_for_map_link(
    int option_list_index, unsigned short maplink_spectile, unsigned short maplink_ovrtype) {
...
static void map_link_change_by_opt_list(
    const struct NoDice_header_options *option_list,
    unsigned short maplink_ovrtype,
    unsigned short maplink_index,
    int relative_value) {
...
int gui_map_link_properties(struct NoDice_map_link *link) {
    ...
    if (strcmp(NoDice_the_level.level->layoutlabel, NoDice_config.game.options.warpzone)) {
...

        {
            ...
            if ((special_tile == NULL) || ((special_tile->override_tile.low.options_list_count == 0) &&
                                           (special_tile->override_tile.high.options_list_count == 0))) {
                const struct NoDice_the_levels *target_level = edit_level_find(
                ...
                g_signal_connect(
                    G_OBJECT(target_level_browse),
                    "clicked",
                    G_CALLBACK(gui_map_links_browse_click),
                    (gpointer) &gui_map_links_browse_click_data);
            ...
            if (special_tile != NULL) {
                ...
                if (special_tile->override_tile.low.options_list_count > 0)
                    gui_generate_option_controls(
                        &property_context,
                        vbox,
                        MAPLINKIDX_PACK(MLIO_TILEL, i, index),
                        (link->layout_addr & 0x00FF),
                        &special_tile->override_tile.low,
                ...
                if (special_tile->override_tile.high.options_list_count > 0)
                    gui_generate_option_controls(
                        &property_context,
                        vbox,
                        MAPLINKIDX_PACK(MLIO_TILEH, i, index),
                        (link->layout_addr & 0xFF00) >> 8,
                        &special_tile->override_tile.high,
                        G_CALLBACK(gui_map_links_option_list_change),
                        G_CALLBACK(gui_map_links_option_toggle_change),
                        G_CALLBACK(gui_map_links_option_spin_change));
                ...
                if (special_tile->override_object.low.options_list_count > 0)
                    gui_generate_option_controls(
                        &property_context,
                        vbox,
                        MAPLINKIDX_PACK(MLIO_OBJL, i, index),
                        (link->object_addr & 0x00FF),
                        &special_tile->override_object.low,
                        G_CALLBACK(gui_map_links_option_list_change),
                        G_CALLBACK(gui_map_links_option_toggle_change),
                        G_CALLBACK(gui_map_links_option_spin_change));
                ...
                if (special_tile->override_object.high.options_list_count > 0)
                    gui_generate_option_controls(
                        &property_context,
                        vbox,
                        MAPLINKIDX_PACK(MLIO_OBJH, i, index),
                        (link->object_addr & 0xFF00) >> 8,
                        &special_tile->override_object.high,
                        G_CALLBACK(gui_map_links_option_list_change),
                        G_CALLBACK(gui_map_links_option_toggle_change),
                        G_CALLBACK(gui_map_links_option_spin_change));
        ...
        g_signal_connect(
            G_OBJECT(world_spin),
            "value-changed",
            G_CALLBACK(gui_map_warpzone_spin_val_change),
            (gpointer) link);
...
static void gui_new_level_tileset_change(GtkComboBox *widget, gpointer user_data) {
    ...
    for (level = 0; level < tileset->levels_count; level++) {
        ...
        snprintf(
            lfp_layout->label_file,
            sizeof(lfp_layout->label_file) - 1,
            "%s / %s (%s)",
            lfp_layout->label,
            lfp_layout->file,
            this_level->name);
        ...
        snprintf(
            lfp_object->label_file,
            sizeof(lfp_object->label_file) - 1,
            "%s / %s (%s)",
            lfp_object->label,
            lfp_object->file,
            this_level->name);
...
static void gui_new_level_existing_lo_change(GtkComboBox *widget, gpointer user_data) {
    const struct _gui_new_level_existing_lo_change_data *data =
        (const struct _gui_new_level_existing_lo_change_data *) user_data;
...
int gui_new_level_popup() {
    ...
    // Layout container...
    {
        ...
        gtk_label_set_markup(
            ...
            "<b>** WARNING **</b> This will close the level you are currently editing!");
        gtk_box_pack_start(GTK_BOX(GTK_DIALOG(popup)->vbox), warning_label, FALSE, FALSE, 5);

        warning_label = gtk_label_new(NULL);
        gtk_label_set_markup(
            GTK_LABEL(warning_label),
            "<b>Cancel and save</b> if you have any edits you want to keep!");
        gtk_box_pack_start(GTK_BOX(GTK_DIALOG(popup)->vbox), warning_label, FALSE, FALSE, 5);
        ...
        // Connect change signal
        g_signal_connect(
            layout_object_cbs.layouts,
            "changed",
            G_CALLBACK(gui_new_level_existing_lo_change),
            (gpointer) &level_layout);
