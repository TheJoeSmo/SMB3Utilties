...
} PPU_SPR_objects[256] = {{0, 0, 0, 0, NULL}}, PPU_SPR_mobjects[256] = {{0, 0, 0, 0, NULL}}, *PPU_SPR = NULL;

// Based on http://nesdev.parodius.com/pal.txt
static const struct _nes_palette {
    unsigned char r, g, b;
} nes_palette[64] = {
    {117, 117, 117}, {39, 27, 143},   {0, 0, 171},     {71, 0, 159},    {143, 0, 119},   {171, 0, 19},
    {167, 0, 0},     {127, 11, 0},    {67, 47, 0},     {0, 71, 0},      {0, 81, 0},      {0, 63, 23},
    {27, 63, 95},    {0, 0, 0},       {0, 0, 0},       {0, 0, 0},       {188, 188, 188}, {0, 115, 239},
    {35, 59, 239},   {131, 0, 243},   {191, 0, 191},   {231, 0, 91},    {219, 43, 0},    {203, 79, 15},
    {139, 115, 0},   {0, 151, 0},     {0, 171, 0},     {0, 147, 59},    {0, 131, 139},   {0, 0, 0},
    {0, 0, 0},       {0, 0, 0},       {255, 255, 255}, {63, 191, 255},  {95, 151, 255},  {167, 139, 253},
    {247, 123, 255}, {255, 119, 183}, {255, 119, 99},  {255, 155, 59},  {243, 191, 63},  {131, 211, 19},
    {79, 223, 75},   {88, 248, 152},  {0, 235, 219},   {0, 0, 0},       {0, 0, 0},       {0, 0, 0},
    {255, 255, 255}, {171, 231, 255}, {199, 215, 255}, {215, 203, 255}, {255, 199, 255}, {255, 199, 219},
    {255, 191, 179}, {255, 219, 171}, {255, 231, 163}, {227, 255, 163}, {171, 243, 191}, {179, 255, 207},
...
static void ppu_init_objs(struct _PPU_SPR *ppu_spr_base, int is_map_objects) {
    ...
    for (i = 0; i < 256; i++) {
        struct NoDice_objects *this_obj = (!is_map_objects) ? &NoDice_config.game.regular_objects[i]
...
void ppu_configure_for_level() {
    ...
    NoDice_config.game.objects = (NoDice_the_level.tileset->id > 0) ? NoDice_config.game.regular_objects
                                                                    : NoDice_config.game.map_objects;
...
void ppu_draw(int x, int y, int w, int h) {
    ...
    if (!NoDice_the_level.header.is_vert) {
        // Level / Worldmap
        max_row = (NoDice_the_level.tileset->id != 0) ? (SCREEN_BYTESIZE / TILESIZE)
                                                      : (SCREEN_MAP_ROW_OFFSET + (SCREEN_BYTESIZE_M / TILESIZE));
