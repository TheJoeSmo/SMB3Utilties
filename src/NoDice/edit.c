    ...
    if (NoDice_the_level.tileset->id > 0) {
        ...
        if (save_layout) {
            ...
            snprintf(
                path_buffer,
                PATH_MAX,
                SUBDIR_LEVELS "/%s/%s" EXT_ASM,
                NoDice_the_level.tileset->path,
                edit_cur_level->layoutfile);
            ...
            if (!asm_file) {
                ...
                snprintf(
                    error_buf,
                    sizeof(error_buf),
                    "edit_level_save: Failed to write %s: %s",
                    path_buffer,
                    strerror(errno));
            ...
            level_alternate = edit_level_find(
        ...
        for (i = 2; i <= 4; i++) {
            fprintf(
                asm_file,
                "(W%s_ByRowType_S%i - W%s_ByRowType)",
...

        {
            ...
            for (col = 0; col < LBL_TOTAL; col++) {
                if ((col == LBL_BYROWTYPE) || (col == LBL_BYSCRCOL) ||
                    ...
                    for (i = 0; i < NoDice_the_level.map_link_count; i++) {
                        ...
                        if (col == LBL_BYROWTYPE) {
                            ...
                            unsigned char tile_id =
                                edit_maptile_get(((link->row_tileset & 0xF0) >> 4) - MAP_OBJECT_BASE_ROW, link->col_hi);

                            // Need to find the level to insert the label
                            const struct NoDice_the_levels *level;

                            // Find out if this is a "special" tile...
                            for (tile_check = 0; tile_check < NoDice_config.game.total_map_special_tiles;
                                 tile_check++) {
        ...
        if (add_layout) {
            ...
            if (!asm_file) {
                ...
                snprintf(
                    error_buf,
                    sizeof(error_buf),
                    "edit_level_add_labels: Failed to write %s: %s",
                    path_buffer,
                    strerror(errno));
                gui_display_message(1, error_buf);
                return 0;
...
static int undo_stack_pos = 0,  // Position within stack
...
static void undo_mark_map_tile(int row, int col, unsigned char tile) {
    struct edit_undo *undo = &undo_stack[undo_stack_pos];
    struct NoDice_the_level_object *map_tile =
        (struct NoDice_the_level_object *) malloc(sizeof(struct NoDice_the_level_object));
...
static void level_gen_insert(
    struct NoDice_the_level_generator *insert_after, struct NoDice_the_level_generator *insert) {
...
static void edit_obj_sort() {
    qsort(
        NoDice_the_level.objects,
        NoDice_the_level.object_count,
        sizeof(struct NoDice_the_level_object),
        sort_objs_compare);
    gui_update_for_generators();
}
...
int edit_obj_insert_object(const struct NoDice_objects *obj, int row, int col) {
    ...
    if (NoDice_the_level.object_count < max) {
        ...
        new_obj->id =
            obj - ((NoDice_the_level.tileset->id > 0) ? NoDice_config.game.objects : NoDice_config.game.map_objects);
        new_obj->row = row;
        new_obj->col = col;
...
static void edit_link_sort() {
    qsort(
        NoDice_the_level.map_links,
        NoDice_the_level.map_link_count,
        sizeof(struct NoDice_map_link),
        sort_links_compare);
    gui_update_for_generators();
}
