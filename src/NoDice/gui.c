...
const char *gui_start_widgets_desc_by_state[] = {
    "Select screen section that Player will junction from (highlighted in green) and click 'Place Start'",
    "Use right mouse button to horizontally place the start position and controls above to configure it.\n\nVertical "
    "start on 'vertical' levels will either start on absolute top or absolute bottom."};
...
static void gui_fixed_calc_size() {
    ...
    if (NoDice_the_level.tiles == NULL) {
        ...
        h = (int) (
            ...
            (double) ((NoDice_the_level.header.total_screens + 1) * TILESIZE * SCREEN_BYTESIZE_V / SCREEN_WIDTH) *
            gui_draw_info.zoom);
    }
...
static void gui_statusbar_update() {
    ...
    if (NoDice_the_level.tileset->id > 0)
        snprintf(
            path_buffer,
            PATH_MAX,
            "Objects: %i / %i   Bank free space: %i bytes",
...
static void menu_view_hint_select(gpointer callback_data, guint callback_action, GtkWidget *menu_item) {
...
static void menu_file_save(GtkWidget *widget, gpointer callback_data) {
    ...
    const struct NoDice_the_levels *current_alt =
        edit_level_find(hdr->alt_level_tileset, hdr->alt_level_layout, hdr->alt_level_objects);
...
static void gui_select_handler(const GtkSelectableOverlay *selectable_overlay) {
    ...
    if (selectable_overlay->gen != NULL) {
        ...
        for (i = 0; i < NoDice_the_level.tileset->gen_count; i++) {
            ...
            if ((tileset_gen->id == selectable_overlay->gen->id) &&
                (tileset_gen->type == selectable_overlay->gen->type)) {
                int p;
                ...
                for (p = 0; p <= selectable_overlay->gen->size - 3; p++) {
                    gtk_spin_button_set_value(
                        GTK_SPIN_BUTTON(gui_parameter_widgets[p].spin),
                        selectable_overlay->gen->p[p]);
                }
...
enum SPECOBJ_OPS { SPECOBJ_PROP, SPECOBJ_ADD, SPECOBJ_DELETE };
...
static void gui_load_tile_hint(const struct NoDice_tilehint *hint, int is_global) {
    ...
    if ((surface = gui_surface_from_file(path_buffer)) != NULL) {
        ...
        snprintf(
            path_buffer,
            sizeof(path_buffer),
            "Failed to load tile hint icon " SUBDIR_ICON_TILES "/%s",
            hint->overlay);
            ...
            if (gui_start_widgets.edit_notebook_page == ENPAGE_GENS) {
                ...
                if (gui_selected_insert_obj != NULL) {
                    if (!edit_obj_insert_object(gui_selected_insert_obj, row, col)) {
                        gui_display_message(
                            TRUE,
                            "You cannot insert any more objects -- maximum for this object set has been reached!");
                    }
                }
...
static gboolean gui_notebook_selected(GtkNotebook *notebook, gpointer arg1, guint arg2, gpointer user_data) {
    ...
    gtk_widget_set_sensitive(
        menu_find_item(gui_menu, "/_Edit/_Delete"),
        (arg2 != ENPAGE_STARTS && arg2 != ENPAGE_TILES));
    ...
    gtk_widget_set_sensitive(
        menu_find_item(gui_menu, "/_Edit/Level _Properties"),
        (arg2 != ENPAGE_TILES) && (arg2 != ENPAGE_MOBJS));
...
const char *gui_make_image_path(const struct NoDice_tileset *tileset, const struct NoDice_the_levels *level) {
    ...
    snprintf(
        path_buffer,
        sizeof(path_buffer) - 5,
        SUBDIR_ICON_LEVELS "/%s_%s_%s",
        tileset->name,
        level->layoutlabel,
        level->objectlabel);
...
static void gui_starts_properties_foreach_callback(GtkWidget *widget, gpointer relative_index_ptr) {
    ...
    unsigned char jct_y =
        NoDice_the_level.Level_JctYLHStart[gui_combobox_simple_get_index(gui_start_widgets.screen_list)];
...
static void gui_screen_list_change(GtkComboBox *widget, gpointer user_data) {
    ...
    gtk_container_foreach(
        GTK_CONTAINER(gui_start_widgets.property_vbox),
        gui_starts_properties_foreach_callback,
        &relative_index);
}
...
static void gui_starts_button_next(GtkButton *button, gpointer user_data) {
    ...
    if (gui_start_widgets.state == 0) {
        ...
        if (!is_cancel) {
            ...
            NoDice_the_level.Level_JctXLHStart[screen] =
                (gui_start_widgets.jct_x & 0xF0) | ((gui_start_widgets.jct_x & 0xF00) >> 8);
    ...
    gtk_text_buffer_insert_at_cursor(
        buffer,
        gui_start_widgets_desc_by_state[gui_start_widgets.state],
        strlen(gui_start_widgets_desc_by_state[gui_start_widgets.state]));
}
...
static void gui_create_right_pane(GtkWidget *vbox) {
    ...
    // Generators tab
    {
        ...
        // Parameters
        {
            ...
            for (i = 0; i < GEN_MAX_PARAMS; i++) {
                ...
                g_signal_connect(
                    G_OBJECT(gui_parameter_widgets[i].spin),
                    "value-changed",
                    G_CALLBACK(gui_parameter_spin_val_change),
                    GINT_TO_POINTER(i));
            }
        ...
        gtk_notebook_append_page(
            ...
            vbox_gens,
            gtk_label_new(edit_notebook_page_names[ENPAGE_GENS]));
    }
    ...
    // Objects tab
    {
...

        {
            ...
            g_signal_connect(
                G_OBJECT(button),
                "clicked",
                G_CALLBACK(gui_special_objs_properties),
                GINT_TO_POINTER(SPECOBJ_PROP));
            ...
            g_signal_connect(
                ...
                "clicked",
                G_CALLBACK(gui_special_objs_properties),
                GINT_TO_POINTER(SPECOBJ_DELETE));
            gtk_box_pack_start(GTK_BOX(hbox_buttons), button, FALSE, FALSE, 2);
        ...
        gtk_notebook_append_page(
            GTK_NOTEBOOK(gui_notebook),
            vbox_objs,
            gtk_label_new(edit_notebook_page_names[ENPAGE_OBJS]));
    }
    ...
    // Start Positions tab
    {
        ...
        g_signal_connect(
            G_OBJECT(gui_start_widgets.button_active),
            "clicked",
            G_CALLBACK(gui_starts_button_next),
        ...
        g_signal_connect(
            G_OBJECT(gui_start_widgets.button_cancel),
            "clicked",
            G_CALLBACK(gui_starts_button_next),
        ...
        gui_generate_option_controls(
            &gui_start_pos_props_context,
            gui_start_widgets.property_vbox,
            0,
            0,
            &NoDice_config.game.jct_options,
            G_CALLBACK(gui_starts_properties_option_list_change),
            G_CALLBACK(gui_starts_properties_option_toggle_change),
            G_CALLBACK(gui_starts_properties_option_spin_change));
        ...
        gtk_notebook_append_page(
            GTK_NOTEBOOK(gui_notebook),
            vbox,
            gtk_label_new(edit_notebook_page_names[ENPAGE_STARTS]));
    }
    ...
    // World map tiles
    {
        ...
        for (tile = 0; tile < 255; tile++) {
            ...
            g_signal_connect(
                G_OBJECT(map_tile_buttons.buttons[tile]),
                "toggled",
                G_CALLBACK(gui_map_tiles_toggled),
                GINT_TO_POINTER(tile));
            g_signal_connect(
                G_OBJECT(map_tile_buttons.buttons[tile]),
                "expose_event",
                G_CALLBACK(gui_map_tiles_expose_event_callback),
                GINT_TO_POINTER(tile));
        ...
        gtk_notebook_append_page(
            GTK_NOTEBOOK(gui_notebook),
            scrolled_window,
            gtk_label_new(edit_notebook_page_names[ENPAGE_TILES]));
    }
    ...
    // World map objects
    {
...

        {
            ...
            g_signal_connect(
                G_OBJECT(button),
                "clicked",
                G_CALLBACK(gui_map_object_properties),
                GINT_TO_POINTER(SPECOBJ_PROP));
            gtk_box_pack_start(GTK_BOX(hbox_buttons), button, FALSE, FALSE, 2);
        ...
        gtk_notebook_append_page(
            GTK_NOTEBOOK(gui_notebook),
            vbox,
            gtk_label_new(edit_notebook_page_names[ENPAGE_MOBJS]));
    }
    ...
    // World map links
    {
        ...
        gtk_notebook_append_page(
            GTK_NOTEBOOK(gui_notebook),
            vbox,
            gtk_label_new(edit_notebook_page_names[ENPAGE_LINKS]));
    }
    ...
    // Menu power v-box
    {
        ...
        // Mid
        {
            ...
            // Left pane
            {
                ...
                g_signal_connect(
                    G_OBJECT(gui_fixed_view),
                    "expose_event",
                    G_CALLBACK(gui_PPU_portal_expose_event_callback),
                    NULL);
...
static void gui_do_load_complete_actions() {
    ...
    gtk_notebook_set_current_page(
        GTK_NOTEBOOK(gui_notebook),
        (NoDice_the_level.tileset->id != 0) ? ENPAGE_GENS : ENPAGE_TILES);
