...
#ifdef LSB_FIRST
    return ((s32 & 0xFF000000) >> 24) | ((s32 & 0x00FF0000) >> 8) | ((s32 & 0x0000FF00) << 8) |
           ((s32 & 0x000000FF) << 24);
#else
    return s32;
#endif
...
} * head_music_tracks[MUSICTRACK_TOTAL], *cur_track_event[MUSICTRACK_TOTAL] = {NULL};
...
static void music_pad_rests(
    struct music_segment *segment,
    const unsigned char *rest_table_base_ptr,
    unsigned char cur_patch,
    unsigned char rest_length,
    int is_noise) {
...
static void music_pad_rests_for_segment_end(
    struct music_segment *segment,
    const unsigned char *rest_table_base_ptr,
    unsigned char cur_patch,
    int rest_length,
...
int MusConv_Import(const char *input_midi_name, const char *output_asm_name) {
            ...
            switch (process_event) {
                ...
                case 0xC0:  // PROGRAM CHANGE
                    ...
                    if ((track - 1) != MUSICTRACK_SQUARE1 && (track - 1) != MUSICTRACK_SQUARE2) {
                        printf(
                            "Warning: Program changes supported only on track 1 (Square 2) and track 2 (Square 1)!\n");
                        break;
                    }
                    ...
                    if (len == 7) {
                        ...
                        if (!strncmp(buffer, "STOP", 4)) {
                            if (track == 0) {
                                ...
                                if (stop_time == 0xFFFF) {
                                    printf(
                                        "WARNING: STOP SysEx detected; will stop processing at %f\n",
                            ...
                            if ((track - 1) == MUSICTRACK_NOISE || (track - 1) == MUSICTRACK_DMC) {
                                ...
                                printf(
                                    "WARNING: LOOP SysEx only applies to Noise and DCM tracks; ignoring on track %i.\n",
                                    track);
                        }
                    } else {
                ...
                default:
                    ...
                    printf(
                        "Error: Don't know how to handle event %02X (%02X, %02X) on track %d!\n",
                        midi_event,
                        byte1,
                        byte2,
                        track);
    ...
    // Post-processing
    {
        ...
        unsigned char cur_patch_per_track[MUSICTRACK_TOTAL] =
            {0x80, 0x80, 0x80, 0x80, 0x80};  // Really only matters for Square 1 and 2
        ...
        for (i = 0; i < segment_total; i++) {
            ...
            unsigned int last_segment_end = (i > 0) ? music_segments[i].start_time : 0;
                ...
                while ((events != NULL) &&                                // ... we run out ...
                    ...
                    if (events->event == EVENT_LOOP) {
                        ...
                        music_pad_rests(
                            ...
                            rest_table_base_ptr,
                            cur_patch,
                            events->time - time_last_event,
                            track == MUSICTRACK_NOISE);
                    ...
                    if (!track_en[track]) {
                        ...
                        if (segment->tracks[track].track_end_time != end_time) {
                            ...
                            if (track != MUSICTRACK_SQUARE2) {
                                ...
                                if (pad_ok)
                                    music_pad_rests_for_segment_end(
                                        segment,
                                        rest_table_base_ptr,
                                        0x80,
                                        time_diff,
                        ...
                        if ((current_end < new_end) && (track < MUSICTRACK_DMC)) {
                            ...
                            memmove(
                                &segment->segment_data[next_track_start + copy_size],
        ...
        for (i = 0; i < segment_total; i++) {
            ...
            fprintf(
                out,
                "\t\tMusSeg $%02X, MXXSegData%02X, $%02X, $%02X, $%02X, $%02X\n",
                rest_table_base,
                i,
